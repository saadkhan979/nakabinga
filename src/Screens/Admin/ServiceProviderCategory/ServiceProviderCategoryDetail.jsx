...
const ServiceProviderCategoryDetail = ({
  filters,
  setFilters,
  pagination,
  updatePagination,
}) => {
  usePageTitle('Category Detail');
    ...
    <div>
      <div className="d-flex align-items-start mb-4 justify-content-between flex-wrap">
        <div className="d-flex flex-column gap-2">
          <h2 className="screen-title m-0 d-inline"><BackButton /> Category Detail</h2>
        </div>
      </div>
        ...
        <div className='row'>
          <div className='col-md-12'>
            <div className='d-inline-block'>
              <Link to={`/admin/service-provider-category/edit/${user?.id}`} className='customButton'>Edit Category</Link>
            </div>
          </div>
      ...
      <CustomModal
        ...
        variant="success"
        title="Success"
        description={`Category has been ${user?.is_active === 0 ? 'Inactivated' : 'Activated'
          } Successfully.`}
      />
...
export default withModal(withFilters(ServiceProviderCategoryDetail));
