...
const EbookDetail = ({
  ...
  const { mutate: updateStatusMutation, isPending: isStatusUpdating } =
    useMutation({
      mutationFn: async () => await updateStatusEbooks(id),
      onSuccess: (data) => {
    ...
    <div>
      <div className="d-flex align-items-start mb-4 justify-content-between flex-wrap">
        <div className="d-flex flex-column gap-2">
          <h2 className="screen-title m-0 d-inline">
            <BackButton url="/admin/content-management" state={{ activeTab: 'ebooks' }} />
            Ebook Detail</h2>
        </div>
      </div>
      <div className="d-card py-45 mb-45">
        <div className="d-flex flex-wrap-reverse justify-content-between">
          <div className="mb-3">
            <p className="text-label">E-Books Title:</p>
            <p className="text-data">{user?.title}</p>
          </div>
        ...
        <div className="row">
          <div className="col-md-6 mb-3">
            ...
            <p className="text-data">{user?.language.name}</p>
          </div>
        </div>
        {/* <div className="row">
          <div className="col-md-6 mb-3">
        ...
        <div className="row">
          {/* <div className="col-md-6 mb-3">
            <div className="categoryVideo border">
              {user?.video ? (
                <video
                  src={user.video}
                  controls
          ...
          <div className="col-md-6 mb-3">
            <div className="categoryFile border d-flex flex-column align-items-center justify-content-center text-center p-3 rounded">
              {user?.file ? (
                <>
                  <p className="fw-bold mb-2">E-Book File</p>
                  <a
                    href={user.file}
                    download
                    target="_blank"
                  ...
                  <p className="mt-2 text-muted small">
                    {user.file.split('/').pop()} {/* Shows file name */}
                  </p>
                </>
              ) : (
        ...
        <div className='row'>
          <div className='col-md-12'>
            <div className='d-inline-block'>
              <Link to={`/admin/content-management/edit-ebooks/${user?.id}`} className='customButton'>Edit</Link>
            </div>
          </div>
      ...
      <CustomModal
        ...
        variant="success"
        title="Success"
        description={`E-Books has been ${user?.is_active === 0 ? 'Inactivated' : 'Activated'
          } Successfully.`}
      />
...
export default withModal(withFilters(EbookDetail));
