...
const ArticlesDetail = ({
  ...
  const { mutate: updateStatusMutation, isPending: isStatusUpdating } =
    useMutation({
      mutationFn: async () => await updateStatusArticles(id),
      onSuccess: (data) => {
        showToast('Status updated successfully', 'success');
        setChangeStatusModal(false);
        setShowSuccessModal(true);
        queryClient.invalidateQueries(['articlesDetails', id]);
      },
      onError: (error) => {
    ...
    <div>
      ...
      <div className="d-card py-45 mb-45">
        <div className="d-flex flex-wrap-reverse justify-content-between">
          <div className="mb-3">
            <p className="text-label">Articles Title:</p>
        ...
        <div className='row'>
          <div className='col-md-12'>
            <div className='d-inline-block'>
              <Link to={`/admin/content-management/edit-articles/${user?.id}`} className='customButton'>Edit</Link>
            </div>
          </div>
      ...
      <CustomModal
        ...
        variant="success"
        title="Success"
        description={`Article has been ${user?.is_active === 0 ? 'Inactivated' : 'Activated'
          } Successfully.`}
      />
...
export default withModal(withFilters(ArticlesDetail));
