...
import {
    getListingAudioCategory,
    updateStatusAudioCategory,
...
import { audioCategoryManagementHeaders } from '../../../Utils/Constants/TableHeaders';
import { formatDate, serialNum, showErrorToast } from '../../../Utils/Utils';
import CustomButton from '../../../Components/CustomButton';

const AudioCategoryManagement = ({
    showModal,
    closeModal,
    filters,
    setFilters,
    ...
    usePageTitle('Audio Category Management');
    ...
    //GET USERS
    const {
        data: audioCatrgoryManagement,
        isLoading,
        isError,
        error,
        refetch,
    } = useFetchTableData(
        'audioCatrgoryManagement',
        filters,
        updatePagination,
        getListingAudioCategory
    );

    const audioCatrgoryManagementdata = audioCatrgoryManagement?.data ?? []
    ...
    const { mutate: updateStatusMutation, isPending: isStatusUpdating } =
        useMutation({
            mutationFn: async (id) => await updateStatusAudioCategory(id),
            onSuccess: (data) => {
        ...
        <>
            <section>
                <div className="d-flex justify-content-between flex-wrap mb-3">
                    <h2 className="screen-title mb-0">Audio Category Management</h2>
                ...
                <Row>
                    <Col xs={12}>
                        <CustomTable
                            ...
                            headers={audioCategoryManagementHeaders}
                        ...
                        >
                            {(audioCatrgoryManagementdata?.length || isError) && (
                                <tbody>
                                    {isError && (
                                        <tr>
                                            <td colSpan={audioCategoryManagementHeaders.length}>
                                    ...
                                    {audioCatrgoryManagementdata?.map((item, index) => (
                                        <tr key={item.id}>
                                            <td>
                                                {serialNum(
            ...
            <CustomModal
                ...
                variant="success"
                title="Success"
                description={`Audio Category has been ${isStatusActive(selectedObj) ? 'Inactivated' : 'Activated'
                    } Successfully.`}
            />
        </>
    );
...
export default withModal(withFilters(AudioCategoryManagement));
